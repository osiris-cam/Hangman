...
int main(int argc, char **argv) {
	...
	while ( num_correct < word_length ) { //while the number of hits is less than the number of letters in the word:
		...
		if( strncmp(guess, "quit", 4) == 0) { //This will compare the first 4 characters, but in an if-else
			quit = 1; //change the value of quit
			break;
		}
		char_entered = guess[0];
		reguessed = 0;
